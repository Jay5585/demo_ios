    ...
    class func getError(responseObject: AnyObject , task : APITask) -> BaseError {
        ...
        if let code = responseObject["code"] as? Int {
            error.errorCode = String(code)
             ...
             error.errorCode = (responseObject["code"] as? String)!
        }
        
        if error.errorCode == "409" {
            AppUtility.getAppDelegate().window?.makeToast(error.alertMessage, duration: 0.5, position: ToastManager.shared.position, title: nil, image: nil, style: ToastManager.shared.style, completion: { (tap) in
