...
//        self.layer.shadowPath = UIBezierPath(roundedRect: layer.bounds, cornerRadius: layer.cornerRadius).cgPath
    ...
    func handleNetworkCheck(isAvailable : Bool, insideView : UIView){
        AppUtility.executeTaskInMainQueueWithCompletion { [weak self] in
            if self == nil{
                return
            }
            if isAvailable {
                self!.showProgressHUD(viewController: insideView, title: nil, subtitle: nil)
            }
            else {
                self!.isLoadedRequest = false
                self!.endEditing(true)
                self!.makeToast(ErrorMessage.noInternet)
            }
        }
    }
    ...
    func showProgressHUD(viewController from: UIView,title: String?,subtitle: String?)
    {
        ...
        if progressHUDView != nil {
            ...
            progressHUDView.dimBackground = false
