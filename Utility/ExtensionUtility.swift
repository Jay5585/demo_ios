...
public extension String {
    ...
    public func countEmojiCharacter() -> Int {
...
public extension UISlider {
    public func setCommonProperty() {
        self.minimumValue = 0
        self.maximumValue = 100
        
        self.minimumTrackTintColor = Color.appIntermidiateBG.value
        self.maximumTrackTintColor = Color.appSecondaryBG.value
        self.thumbTintColor = Color.appSecondaryBG.withAlpha(0.45)
...
{
    public func getUTCTimeStampe() -> TimeInterval {
    ...
    //Remove seconds from given UTC time
    public func getTimeStampOfGivenTime(time : String)  -> TimeInterval? {
        ...
        let onlyDate = dateFormatter1.string(from: date)
        ...
        if let d : Date = dateFormatter.date(from: "\(onlyDate) \(time)") {
            return Date(timeInterval: 0, since: d).timeIntervalSince1970
        }
        else {
            return nil
        }
    ...
    public func getOnlyTimeofGivenTimestamp(timestampe : TimeInterval)  -> String {
...
public extension NSObject
{
    public func setValueFromDictionary(_ dicResponse : NSDictionary) {
        ...
        for key in allKey {
            if let value = dicResponse.value(forKey: key), !(value is NSNull) {
